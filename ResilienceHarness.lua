-- Resilience Harness v3.9 — Compact Rail UI (Safe, Measurement-Only)
-- Tight gaps, minimal layers, left rail fixed, ripple+shadow on all controls.
...
--============= SAFE MEASUREMENT CORE (unchanged behavior) =============
...
--============= STYLE ===================================================
local GAP=6               -- tiny gap between cards/sections
local R=10                -- radius
local SHADOW_IMG="rbxassetid://13637412666" -- soft glow
local function shadowUnder(ui, inflate)
	local h=new("ImageLabel",{BackgroundTransparency=1,Image=SHADOW_IMG,ImageColor3=rgb(0,0,0),ImageTransparency=0.76,
		ScaleType=Enum.ScaleType.Slice,SliceCenter=Rect.new(64,64,64,64),ZIndex=(ui.ZIndex or 1)-1}, ui)
	h.Size=UDim2.new(1, (inflate or 10), 1, (inflate or 10))
	h.Position=UDim2.fromOffset(-(inflate or 10)/2, -(inflate or 10)/2)
	return h
end
...
-- subtle world blur when open
local blur = Lighting:FindFirstChild("HarnessBlur") or new("BlurEffect",{Name="HarnessBlur",Size=0},Lighting)
local function setBlur(on) TweenService:Create(blur,TweenInfo.new(0.15,Enum.EasingStyle.Sine,Enum.EasingDirection.Out),{Size=on and 10 or 0}):Play() end
...
-- Window (single main layer; clipped so rail/content never spill)
...
-- Left rail (fixed inside; tight)
local rail=new("Frame",{Size=UDim2.fromOffset(100, root.Size.Y.Offset-28-GAP),Position=UDim2.fromOffset(GAP,28+GAP),
	BackgroundColor3=rgb(28,30,42)},root)
...
local railList=new("UIListLayout",{Padding=UDim.new(0,GAP),HorizontalAlignment=Enum.HorizontalAlignment.Center},rail)
...
-- Content (single solid layer; tight margin)
local content=new("Frame",{Size=UDim2.fromOffset(root.Size.X.Offset-rail.Size.X.Offset-(GAP*3), rail.Size.Y.Offset),
	Position=UDim2.fromOffset(rail.Position.X.Offset+rail.Size.X.Offset+GAP, rail.Position.Y.Offset),BackgroundColor3=rgb(24,26,36)},root)
...
-- Card factory (title+desc+toggle)
...
-- Build Main cards (map to safe stress ops)
...
local mPad=new("UIPadding",{PaddingTop=UDim.new(0,GAP),PaddingLeft=UDim.new(0,GAP)},pageMetrics)
local function metric(y,text) return new("TextLabel",{Text=text..": --",Font=Enum.Font.GothamSemibold,TextSize=13,TextXAlignment=Enum.TextXAlignment.Left,TextColor3=rgb(235,240,255),BackgroundTransparency=1,Position=UDim2.fromOffset(4,y),Size=UDim2.fromOffset(200,18)},pageMetrics) end
...
local dd = tinyBtn("Scenario: "..scenario, content.Size.X.Offset-(GAP*2))
dd.Position=UDim2.fromOffset(0,0)
local ddMenu=new("Frame",{Visible=false,BackgroundColor3=rgb(26,28,40),Position=UDim2.fromOffset(0,28+GAP),Size=UDim2.fromOffset(dd.Size.X.Offset, (#SCENARIOS*24)+8),ZIndex=content.ZIndex+2},pageConfig)
...
for _,name in ipairs(SCENARIOS) do
	local it=tinyBtn(name, dd.Size.X.Offset-12); it.Parent=ddMenu
...
local anchorBtn=tinyBtn("Reset Path Anchor")
anchorBtn.Position=UDim2.fromOffset(0, 28+GAP)
...
local runScenarioBtn=tinyBtn("Run Scenario")
runScenarioBtn.Position=UDim2.fromOffset(0, 28+GAP+28+GAP)
runScenarioBtn.MouseButton1Click:Connect(function() appendLog("Run: "..scenario); Telemetry.push({t=time(),tag="SCENARIO_START",name=scenario}); (Campaign[scenario] or function() end)(); Telemetry.push({t=time(),tag="SCENARIO_END",name=scenario}); appendLog("Done: "..scenario) end)
local exportBtn=tinyBtn("Export JSON")
exportBtn.Position=UDim2.fromOffset(0, 28+GAP+28+GAP+28+GAP)
exportBtn.MouseButton1Click:Connect(function() local j=HttpService:JSONEncode(Telemetry.buffer); print("-- EXPORT BEGIN --"); print(j); print("-- EXPORT END --"); appendLog(("Exported %d events"):format(#Telemetry.buffer)) end)
...
-- Minimize → small pill; restore toggles blur back on
local minimized=false; local pill
local function restore() if pill then pill:Destroy(); pill=nil end; root.Visible=true; setBlur(true); minimized=false end
local function minimize() setBlur(false); root.Visible=false; minimized=true; pill=new("TextButton",{Text="RH",Font=Enum.Font.GothamSemibold,TextSize=12,TextColor3=rgb(22,26,34),AutoButtonColor=false,BackgroundColor3=rgb(140,230,190),Size=UDim2.fromOffset(36,20),Position=UDim2.fromOffset(24,24)},gui); new("UICorner",{CornerRadius=UDim.new(1,0)},pill); shadowUnder(pill,10); clickFX(pill); pill.MouseButton1Click:Connect(restore) end
...
RunService.Heartbeat:Connect(function(dt)
	...
	if time()-Telemetry.lastFlush>=Telemetry.flushInterval then
		...
		print("-- TELEMETRY FLUSH --", HttpService:JSONEncode({n=#Telemetry.buffer,last=Telemetry.buffer[#Telemetry.buffer]}))
	end
end)
